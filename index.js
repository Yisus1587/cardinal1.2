...
▢ ${msg.gp} 1
https://instabio.cc/4052514aPykQW

▢ *YouTube*
• sin canal por el momento
`
m.reply(gps)
break 
...
case 'creador': 
case 'developer':
   //CARDINAL.sendContact(from, '5213324206574', 'CARDINAL', mek) 
   m.reply(`
*≡ OWNER*
...
▢ *Pregunta:* ${value}
▢ *Respuesta :* ${prefg.success.replace('simsimi', 'CARDINAL').replace('Simsimi', 'CARDINAL').replace('sim simi', 'CARDINAL')}`) 
break 
...
Fg.groupSettingChange(from, GroupSettingChange.messageSend, true)
 ...
 case 'view':
   ...
   try {
	      ...
	      ppimg = 'https://ibb.co/wskV9Qh][img]https://i.ibb.co/YZCmzdB/FB-IMG-1653398304124.jpg';
	    }
...
case 'bot':
        ...
        m.reply(result.success.replace('simsimi', 'CARDINAL').replace('Simsimi', 'CARDINAL').replace('sim simi', 'CARDINAL'))
                     break
...
if(!isCmd && isChatbot === true){
 ...
 // if(!isPremium) return
  ...
  m.reply(result.success.replace('simsimi', 'DyLux').replace('Simsimi', 'DyLux').replace('sim simi', 'CARDINAL'))
}
